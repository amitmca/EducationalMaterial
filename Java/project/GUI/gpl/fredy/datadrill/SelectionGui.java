...
public class SelectionGui extends javax.swing.JPanel {
    ...
    private void closeCon() { con.closeCon(); }
    ...
    private JPanel buttonPanel() {
        ...
        view.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                gpl.fredy.sqltools.sqlTable sqt = new gpl.fredy.sqltools.sqlTable(getCon(),getQuery(),false,false);
        ...
        exec.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                gpl.fredy.sqltools.DataExportGui dataExportGui =
                        new gpl.fredy.sqltools.DataExportGui(getCon().con,
                        getQuery(),
                        false,true);
        ...
        displayQuery.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                gpl.fredy.ui.TextEditor te = new gpl.fredy.ui.TextEditor(getQuery());
    ...
    private JPanel selectListe() {
        ...
        origQ.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                gpl.fredy.ui.TextEditor te = new gpl.fredy.ui.TextEditor(getOrigQuery());
                te.ok.addActionListener(new ActionListener() {
                    public void actionPerformed(ActionEvent e) {
                        frame.dispose();
                    }
    ...
    private void loadQuery() {
        ...
        if ( fileName != null ) {
            ...
            if ( doc != null ) {
                ...
                /**
                 * there is no need to parse again, because the
                 * JComboBox is throwing an event and therefore parses
                 * the file by itself
                 **/
    ...
    public Vector getAllComponents() {
        ...
        for (int i=0;i <  c.length;i++) {
            if ( c[i] instanceof SelectionGui ) {
                ...
                for (int j = 0; j < v2.size();j++) v.addElement(v2.elementAt(j));
            } else {
                v.addElement(c[i]);
            }
        }
    ...
    public String getData() { return getQuery(); }
        ...
        for (int i = 0; i < sqlStatement.size();i++) {
            ...
            if ( ((String) sqlStatement.elementAt(i)).toLowerCase().startsWith(panelTyp.toLowerCase()) ) {
                ...
                while (st.hasMoreTokens() ) {
                    ...
                    boolean a = true;  // AND
                    boolean t = true;  // TEXT
                    if (((String)st.nextToken()).toLowerCase().equals("or") )   a = false;
                    if (((String)st.nextToken()).toLowerCase().equals("zahl") ) t = false;
    ...
    public Element getXMLfromComponent(String n, String type) {
        ...
        for (int i=0;i < c.length;i++) {
            String componentName = c[i].getName();
            if (componentName.equals(n) ) {
                if (type.equals("#execute")) elt = ((SelectPanel)c[i]).getXML();
                if (type.equals("#checkbox")) elt = ((CheckBoxPanel)c[i]).getXML();
                if (type.equals("#criteriabox")) elt = ((CriteriaPanel)c[i]).getXML();
                if (type.equals("#between")) elt = ((BetweenPanel)c[i]).getXML();
        ...
        for (int i = 0; i < sqlStatement.size();i++) {
            ...
            if ( ((String) sqlStatement.elementAt(i)).toLowerCase().startsWith(panelTyp.toLowerCase()) ) {
                ...
                while (st.hasMoreTokens() ) {
                    ...
                    boolean t = true;  // TEXT
                    if (((String)st.nextToken()).toLowerCase().equals("or") )   a = false;
                    if (((String)st.nextToken()).toLowerCase().startsWith("zahl") ) t = false;
    ...
    private void createExampleEnvironment(String startDirectory) {
        try {
            ...
            String bs = "unix";
            if (os.indexOf("windows") > 0 )  bs= "windows";
            
            String p1 = startDirectory + File.separator + "datadrill" + File.separator + "resources" + File.separator ;
            readFromJarAndWriteToFile("gpl/fredy/datadrill/resources/" + bs + "/car-list.display",       p1 + "car-list.display");
            readFromJarAndWriteToFile("gpl/fredy/datadrill/resources/" + bs + "/car-list.other",         p1 + "car-list.other");
            readFromJarAndWriteToFile("gpl/fredy/datadrill/resources/" + bs + "/car-list.sel",           p1 + "car-list.sel");
            readFromJarAndWriteToFile("gpl/fredy/datadrill/resources/" + bs + "/employee-list.display",  p1 + "employee-list.display");
            readFromJarAndWriteToFile("gpl/fredy/datadrill/resources/" + bs + "/employee-list.function", p1 + "employee-list.function");
            readFromJarAndWriteToFile("gpl/fredy/datadrill/resources/" + bs + "/employee-list.sel",      p1 + "employee-list.sel");
            readFromJarAndWriteToFile("gpl/fredy/datadrill/resources/" + bs + "/selections.txt",         p1 + "selections.txt");

...
public class Change extends JInternalFrame implements ItemListener,ActionListener 
 {
    JLabel l1;
    JLabel l2;
    JComboBox dbcombo;
    ...
    JTextField size;
    JComboBox colcombo;	
    public static String db="",tables="",table="",col_name="",datatype="",colname1=""; 	
    ...
    public Change()
	{
	super("Change a Column Name");
	setLocation(275,120);
	ImageIcon i=new ImageIcon("images/b_tblops.PNG");
 	setFrameIcon(i);
        ChangeLayout customLayout = new ChangeLayout();
...
public void actionPerformed(ActionEvent ae)
	...
	if(ae.getSource()==btnalt)
		...
		if(a.equals(""))
			{
			String str3="Please enter New Column Name";
			JOptionPane.showMessageDialog(null,str3,"Error",JOptionPane.INFORMATION_MESSAGE); 		
			}
		else
			{
			if(datatype.equals("DATE") || datatype.equals("TIME") || datatype.equals("DATETIME"))
			 {
			   try
			   {	
			   String q="alter table "+db+"."+table+" change "+colname1+" "+a+" "+datatype;
			...
			try
			  {
  			  String q="alter table "+db+"."+table+" change "+colname1+" "+a+" "+datatype+"("+s+")";
...
class ChangeLayout implements LayoutManager {
    ...
    public void layoutContainer(Container parent) {
        ...
        if (c.isVisible()) {c.setBounds(insets.left+8,insets.top+80,160,24);}
        c = parent.getComponent(5);
        if (c.isVisible()) {c.setBounds(insets.left+8,insets.top+144,168,16);}
        c = parent.getComponent(6);
        if (c.isVisible()) {c.setBounds(insets.left+240,insets.top+168,112,24);}
        c = parent.getComponent(7);
        if (c.isVisible()) {c.setBounds(insets.left+240,insets.top+144,88,16);}
        c = parent.getComponent(8);
        if (c.isVisible()) {c.setBounds(insets.left+8,insets.top+168,152,24);}
        ...
        if (c.isVisible()) {c.setBounds(insets.left+376,insets.top+144,72,16);}
        c = parent.getComponent(14);
        if (c.isVisible()) {c.setBounds(insets.left+384,insets.top+168,48,24);}
